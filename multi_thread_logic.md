...
## mcts模擬的CPU worker
存在一個佇列，裡面放著尚需模擬的樹id
1. 若佇列內有id，負責模擬mcts的CPU worker從佇列拿一個取出一個樹id；若佇列內沒有id，worker進入wait狀態，直到佇列內有id，節省cpu資源(在wait狀態，如果全部的樹完成模擬，則喚醒，工作完成)
2. 嘗試鎖住樹(mutex)，鎖住失敗就跳到步驟1.，成功鎖住的話則開始跑mcts，一棵樹只會由一個執行緒來模擬
2. 模擬的過程，worker會把資料放進local暫存的16個子節點資料的區塊(資料不會和其他樹混合，一次是做16次模擬，待推論子節點數量小於等於16)
3. 完成模擬後，直接將所有節點資料丟進可以被填入的buffer
...
3. 若此樹的模擬次數尚未達到目標次數，則將樹的id放到尚需模擬的佇列；若到達模擬次數，則幫此樹標記為已完成
4. 解鎖
5. 如果有將樹的id放回佇列，喚醒一個worker
6. 檢查是否全部的樹都已完成，如果是，則工作完成
