"""
state/cpp_adapter.py — C++ 後端適配層

包裝 pybind11 C++ 的 PhaseGameState，提供與 PythonPhaseGameState 一致的介面。
"""
...
from core.constants import BOARD_LAYOUT, DIRECTIONS, PHASE_STEP_1, MoveKind, piece_owner
...
_POS_TO_IDX: Dict[Tuple[int, int], int] = {
    pos: i for i, pos in enumerate(_VALID_POSITIONS)
}
...
class _CppBoardSnapshot:
    ...
    def first_occupied_in_direction(
        ...
        rr, cc = r + dr, c + dc
        while (
            0 <= rr < len(BOARD_LAYOUT)
            and 0 <= cc < len(BOARD_LAYOUT[0])
            and BOARD_LAYOUT[rr][cc] != -1
        ):
            pos = (rr, cc)
...
class _CppAIAdapter:
    """包裝 C++ 模組提供的 AI helper 方法。"""

    def __init__(self, owner: CppPhaseGameStateAdapter) -> None:
        self._owner = owner

    def reconstruct_unified_action(self, action_idx: int, phase: str) -> Any:
        """將 C++ 端解碼的動作還原為 Move 物件。"""
        ...
        dst = self._owner.game.board.first_occupied_in_direction(
            src, DIRECTIONS[direction_idx]
        )
        if dst is None:
    ...
    def encode_unified_action_from_move(self, move: Any) -> int:
        """將 Move 物件編碼為 C++ 端理解的動作索引。"""
        ...
        for i, direction in enumerate(DIRECTIONS):
            hit = self._owner.game.board.first_occupied_in_direction(
                move.src, direction
            )
            if hit == move.dst:
                direction_idx = i
        ...
        for edge_idx, edge in enumerate(edge_table):
            if int(edge[0]) == src_idx and int(edge[1]) == direction_idx:
                ...
                return int(
                    self._owner._cpp_module.encode_action_idx(kind_name, edge_idx)
                )
        raise ValueError("(src, direction) not present in fixed edge table")


class CppPhaseGameStateAdapter:
    """C++ PhaseGameState 的 Python 包裝。

    提供與 PythonPhaseGameState 一致的介面（clone / is_done / winner /
    current_player / phase / legal_mask / apply_action），
    內部委派給 pybind11 C++ 物件。
    """

    def __init__(
        self, cpp_module: Any, inner: Optional[Any] = None
    ) -> None:
    ...
    def apply_action(
        self, action_idx: int, legal_mask: Optional[torch.Tensor] = None
    ) -> None:
        _ = legal_mask
