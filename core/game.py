"""
core/game.py — Tzaar 規則引擎

提供：
- Move dataclass（完整動作）
- GameResult dataclass（勝負結果）
- Board 類別（棋盤狀態與基本操作）
- TzaarGame 類別（完整規則引擎）

依賴：core/constants.py（不依賴 torch 或任何 ML 套件）
"""
...
class Board:
    """棋盤：每個有效格存放一疊棋子 (piece_code, height)。"""
    ...
    def __init__(self, layout: Optional[List[List[int]]] = None) -> None:
        ...
        for row in layout:
            ...
            for value in row:
                if value == -1:
                    row_data.append(None)       # 洞位
                elif value == 0:
                    row_data.append((0, 0))     # 空格
                else:
                    row_data.append((value, 1)) # 初始棋子，高度 1
    ...
    def get_cell(self, pos: Tuple[int, int]) -> Optional[Tuple[int, int]]:
        """讀取指定位置 (top_piece, height)。空格回傳 (0, 0)，洞位回傳 None。"""
        r, c = pos
        return self.grid[r][c]

    def set_cell(self, pos: Tuple[int, int], piece_code: int, height: int) -> None:
        """覆寫指定位置的格子。清空格用 set_cell(pos, 0, 0)。"""
        r, c = pos
